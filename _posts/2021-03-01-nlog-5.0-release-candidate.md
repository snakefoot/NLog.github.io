...
requirement will be lazy initialized with retry if necessary.
...
but is now disabled by default with NLog 5.0.
...
dependencies and removes static code-analysis warnings about NLog being a security risk because of the DatabaseTarget.
...
* **Impact:** Property lookup with key `UserName` will now match item-key `username`. This also means it will also overwrite independent
  of casing.
